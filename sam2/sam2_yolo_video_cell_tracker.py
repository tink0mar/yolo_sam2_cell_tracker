...
class SAM2YOLOVideoPredictor(SAM2VideoPredictor):
    ...
    def __init__(
        ...
        mask_correction_iou_threshold: float = 0.7,
        max_age_without_yolo_box: int = 6,
        max_frames_retain_removed_obj: int = 16,
        temp_object_maturity: int = 4,
        max_swap_distance_threshold: float = 100.0,
        ...
        **kwargs,
        ...
        self.mask_correction_iou_threshold=mask_correction_iou_threshold
        self.max_age_without_yolo_box =max_age_without_yolo_box
        self.max_frames_retain_removed_obj = max_frames_retain_removed_obj
        self.temp_object_maturity = temp_object_maturity
        self.max_swap_distance_threshold = max_swap_distance_threshold
    ...
    def _init_tracking_structures(inference_state: dict):
        ...
        inference_state["_removed_objects"] = {}   
    ...
    def _age_removed_objects(self, inference_state: dict):
        ...
        for obj_id, info in removed.items():
            info["age"] += 1
            if info["age"] > self.max_frames_retain_removed_obj:
    ...
    def _evaluate_temporary_objects(
        ...
        for temp_id, info in list(temp_objects.items()):
            ...
            best_orig_id = None
            best_dist = self.max_swap_distance_threshold  
    ...
    def find_containment_pairs(
        ...
        masks_bool = (video_res_masks > 0)[:, 0]              
        flat = masks_bool.view(n_obj, -1).float()             
    ...
    def _apply_corrections(
        ...
        if prev_frame_idx < inference_state["num_frames"]:
            ...
            if prev_masks is not None:
                prev_obj_ids = list(inference_state["obj_ids"])  
        ...
        for obj_id, yolo_idx in all_matches.items():
            ...
            if not other_candidates and _iou_bbox(sam_bbox, box) > self.mask_correction_iou_threshold:
                continue
...
def make_yolo_detector(
    ...
    def detect(frame_rgb: np.ndarray) -> np.ndarray:
        results = model(
            frame_rgb, conf=detection_conf_threshold, classes=classes, device=device, verbose=False
        )
